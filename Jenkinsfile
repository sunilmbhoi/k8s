pipeline {
  ...
  stages {
    stage('Codecompileandpush') {
      steps {
        git credentialsId: 'bc045ec1-269d-465f-9164-eb103d990eaa', url: 'xxxxxx/sampleweb.git', branch: 'QA'
        sh 'curl -v -u admin:admin123 --upload-file target/sampleweb.war xxxxxxx/repository/jenkinsartifacts/sampleweb.war'
      }
     }
    stage('BuildImage') {
      steps {
